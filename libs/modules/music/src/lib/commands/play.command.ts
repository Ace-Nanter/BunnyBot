...
export default class PlayCommand extends AbstractMusicCommand {
  ...
  slashCommand = new SlashCommandSubcommandBuilder()
    ...
    .addStringOption((option) =>
      option.setName('song').setDescription('The name of the song to play or URL').setRequired(true)
    );
  ...
  execution = async (interaction: CommandInteraction): Promise<void> => {
    ...
    if (!interaction.guild || !interaction.member || !song) {
      return Promise.reject(new Error('An error has occurred'));
    }

    this.play(member.voice.channel as VoiceChannel, interaction.guild, member, song).then(
   ...
   */
  private async play(voiceChannel: VoiceChannel, guild: Guild, member: GuildMember, arg: string): Promise<GuildMusic> {
    // Check if they are in a voice channel
    if (!voiceChannel) {
      return Promise.reject(
        EmbedHelper.createColouredEmbed('BLUE', 'You need to be in a voice channel to play music!')
      );
    }
  ...
  private addSongToQueue(song: Song, guild: Guild, voiceChannel: VoiceChannel): GuildMusic {
    let guildMusic: GuildMusic = this.module.guildMusicMap.get(guild.id);
    if (guildMusic === undefined) {
      guildMusic = new GuildMusic(guild, voiceChannel);
      this.module.guildMusicMap.set(guild.id, guildMusic);
    }
